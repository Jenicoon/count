...
import { DAY_LABEL_MAP, GATE_LABEL_MAP, type DayId, type GateId } from "@/lib/constants";

type Props = {
  dayId: DayId;
  gateId: GateId;
  initialRow: CounterRow;
};

export function GateCounterBoard({ dayId, gateId, initialRow }: Props) {
  const [row, setRow] = useState<CounterRow>(initialRow);
  ...
  const currentTotal = useMemo(() => row.entered_count - row.exited_count, [row]);

  async function updateCount(action: "enter" | "exit") {
  ...
  return (
    <div className="stack">
      <div className="toolbar">
        <div className="pill">
          {DAY_LABEL_MAP[dayId]} {GATE_LABEL_MAP[gateId]} 현재 인원 {currentTotal.toLocaleString()}명
        </div>
      ...
      <article className="card counter">
        ...
        <div className="button-row">
          <button
            ...
            disabled={isPending && pendingAction === "enter"}
            onClick={() => updateCount("enter")}
          >
            입장 +1
          </button>
          <button
            ...
            disabled={isPending && pendingAction === "exit"}
            onClick={() => updateCount("exit")}
          >
            퇴장 +1
          </button>
        </div>
